...
// Body parsing middleware - ARREGLADO
app.use(
  express.json({
    limit: "10mb",
    // Removido el verify que causaba problemas
  }),
)
...
app.use("*", (req, res) => {
  res.status(404).json({
    ...
    availableRoutes: ["/api/auth", "/api/users", "/api/categories", "/api/products", "/api/orders", "/api/tables"],
  })
})
